...
void frame::setNewAnalyticsText()         //functions like a madlib. Calcs relevant statistics, formats them with proper number of sigfigs, and places into paragraphs.
{
    ...
    betaText->SetLabelText(wxString("Your portfolio’s beta is " + portBeta + " when using the S&P 500 as a proxy of the market. This gives your portfolio a Treynor ratio of " + portTreynor + " compared to " + sandpTreynor + " for the S&P. Consider diversifying your portfolio with stocks that yield more or that have lower betas to improve this ratio."));
    ...
    sharpeText->SetLabelText(wxString("Your portfolio’s Sharpe ratio is " + portShp + " compared to the S&P’s value of " + sandpShp + ".This means your portfolio’s risk/return tradeoff is " + betterOrWorse + " than the S&P 500's. To improve your portfolio’s Sharpe ratio, consider choosing stocks that yield more or that covary less with each other."));
    ...
    stdText->SetLabelText(wxString("Your portfolio’s yearly returns have a standard deviation of " + portStd + " which is " + timesSandp + "x that of the S&P 500 and " + timesRussell + "x that of the Russell 2000. Assuming past returns hold, your portfolio would be worth $" + futureValue + " in " + horizon + " year(s)."));
