...
//     wxBrush semiWhite(image1);
		...
		dc.DrawText(m_name, m_dim.x, m_dim.y);
    }
}
...
void mpBitmapLayer::SetBitmap( const wxImage &inBmp, double ls, double lr)
{
    if (!inBmp.Ok())
    {
...
void mpBitmapLayer::Plot(wxDC & dc, mpWindow & w)
{
    ...
    {
		dc.GetSize(&graphWidth, &graphHeight);
		double adjMinX = 0;
		double adjMinY = 0; 
		wxCoord width = graphWidth / 50.0;
		wxCoord height = graphHeight / 50.0;

		m_min_x = w.x2p(std);
		m_min_y = w.y2p(ret);
		adjMinX = m_min_x - width / 2.0;
		adjMinY = m_min_y - height / 2.0;

		m_scaledBitmap = wxBitmap( 
			wxBitmap(m_bitmap).ConvertToImage()
			.Scale(width, height) );
			
		dc.DrawBitmap( m_scaledBitmap, adjMinX, adjMinY, true );
    }
